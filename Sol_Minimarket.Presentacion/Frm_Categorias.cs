...
using Sol_Minimarket.Presentacion;
...
namespace SQL_Minimarket.Presentacion
{
    public partial class Frm_Categorias : Form
    {
        ...
        {
            if (Dgv_principal.CurrentRow != null) // Ensure a row is selected
            {
                if (string.IsNullOrEmpty(Convert.ToString(Dgv_principal.CurrentRow.Cells["codigo_ca"].Value)))
                {
                    MessageBox.Show("No se tiene información para Visualizar", "Aviso del Sistema", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
                else
                {
        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            ...
            else // Se procedería a registrar la información
            {
                ...
                if (Rpta == "OK")
                {
                    ...
                    Tbp_prinicpal.SelectedIndex = 0;
                    this.Listado_ca("%"); // Refrescar el listado
                }
                else
                {
        ...
        private void Btn_eliminar_Click(object sender, EventArgs e)
        {
            if (Dgv_principal.CurrentRow != null) // Ensure a row is selected
            {
                ...
                {
                    DialogResult Opcion;
                    Opcion = MessageBox.Show("¿Está seguro de eliminar el registro seleccionado?", "Aviso del Sistema", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        {
            Frm_Rpt_Categorias oRpt1 = new Frm_Rpt_Categorias();
        ...
        {
            this.Selecciona_item();
            this.Estado_Botonesprocesos(false); // Added this line based on the image
            Tbp_prinicpal.SelectedIndex = 1;
        }
